...
import robot_status_pb2_grpc

class RobotDashboard:
    def __init__(self):
        # Set up a connection channel to your running C++ Server
        self.channel = grpc.insecure_channel('127.0.0.1:50051')
        self.stub = robot_status_pb2_grpc.RobotStatusServiceStub(self.channel)
        
        # Request configuration data packet
        self.request = robot_status_pb2.RobotStatusRequest(send_status=True)
        
        # Initiate the server streaming response channel iterator
        self.data_stream = self.stub.GetRobotStatus(self.request)
        ...
        self.line, = self.ax.plot([], [], 'r-o', label="Robot Latitude Target Tracker")
    ...
    def update_plot(self, frame):
        try:
            ...
            status_update = next(self.data_stream)
            
            self.frame_counter += 1
            self.x_indices.append(self.frame_counter)
            self.latitudes.append(status_update.latitude)
            ...
            self.line.set_data(self.x_indices, self.latitudes)
    ...
    def run(self):
        ...
        self.ani = FuncAnimation(self.fig, self.update_plot, blit=False, interval=200, cache_frame_data=False)
        plt.title("Live Robot Telemetry Monitor")
        plt.xlabel("Sample Count Updates")
